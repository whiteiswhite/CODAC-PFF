...
userRouter.post('/register',(req, res)=>{
    ...
    User.findOne({username}, (err, user)=>{
        ...
        if(user)
            ...
            newUser.save(err=>{
                if(err)
                    res.status(500).json({message : {msgBody : "Erreur lors de la création du compte", msgError: true}});
                else
                    res.status(201).json({message : {msgBody : "Votre compte a bien été crée !", msgError: false}});
            })
        }
    })
});
